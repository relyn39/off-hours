...
def default_rules_comercial():
    # Seg–Sex: 07–19 | Sáb/Dom: não permitido
    return {
...
def build_effective_rules(uo_rules=None, group_rules=None, plate_rules=None):
    # Base agora é o preset COMERCIAL por padrão
    base = default_rules_comercial()
...
def build_rules_lookup(uos, groups, plates, rules_state):
    ...
    for uo in [u for u in uos if str(u).strip()!=""]:
        # Inicializa como COMERCIAL por padrão
        rules_lookup[("UO", uo)] = rules_state["UO"].get(uo, default_rules_comercial())
    for g in [g for g in groups if str(g).strip()!=""]:
        rules_lookup[("GROUP", g)] = rules_state["GROUP"].get(g, default_rules_comercial())
    for p in plates:
        rules_lookup[("PLATE", p)] = rules_state["PLATE"].get(p, default_rules_comercial())
    return rules_lookup
...
st.caption("Upload de **um ou mais** CSVs. Suporte a DaaS (,) e VFleets (;). Presets de regras: Comercial (padrão, seg–sex 07–19; sáb/dom não permitido), Noturno e 24x7.")
...
if st.session_state["data_pool"]:
    ...
    # Estado inicial das regras e preset atual
    if "rules" not in st.session_state:
        st.session_state["rules"] = {"UO": {}, "GROUP": {}, "PLATE": {}}
    if "preset_name" not in st.session_state:
        st.session_state["preset_name"] = "Comercial"

    # garantir defaults (COMERCIAL) para tudo que surgir
    def ensure_defaults_comercial():
        for u in uos_all_for_rules:
            st.session_state["rules"]["UO"].setdefault(u, default_rules_comercial())
        for g in groups_all_for_rules:
            st.session_state["rules"]["GROUP"].setdefault(g, default_rules_comercial())
        for p in plates_all_for_rules:
            st.session_state["rules"]["PLATE"].setdefault(p, default_rules_comercial())

    ensure_defaults_comercial()

    # Helper: atualizar widgets de horário quando preset muda
    def update_rule_widget_keys():
        for scope_name, scope_dict in st.session_state["rules"].items():
            for item, rules in scope_dict.items():
                for d in range(7):
                    s1_key = f"{scope_name}_{item}_d{d}_s1"
                    e1_key = f"{scope_name}_{item}_d{d}_e1"
                    s2_key = f"{scope_name}_{item}_d{d}_s2"
                    e2_key = f"{scope_name}_{item}_d{d}_e2"
                    r = rules.get(d, [(None,None),(None,None)])
                    st.session_state[s1_key] = (r[0][0] or "")
                    st.session_state[e1_key] = (r[0][1] or "")
                    st.session_state[s2_key] = (r[1][0] or "")
                    st.session_state[e2_key] = (r[1][1] or "")

    # Botões de preset (atualizam regras + campos)
    ...
    def apply_preset(rule_func, label):
        ...
        update_rule_widget_keys()
        st.toast(f"Preset '{label}' aplicado e campos atualizados.", icon="✅")

    with cpr1:
        if st.button("Preset: Comercial (Seg–Sex 07:00–19:00; Sáb/Dom não permitido)"):
            apply_preset(default_rules_comercial, "Comercial")
    with cpr2:
        if st.button("Preset: Noturno (Seg–Sex 18:00–06:00; Sáb/Dom 00:00–23:59)"):
            apply_preset(default_rules_noturno, "Noturno")
    with cpr3:
        if st.button("Preset: 24x7 Permitido (00:00–23:59 todos os dias)"):
            apply_preset(default_rules_24x7, "24x7")

    st.caption(f"Preset atual: **{st.session_state['preset_name']}**")
    ...
    def rules_editor(scope_name, scope_values):
        ...
        rules = st.session_state["rules"][scope_name].setdefault(selected, default_rules_comercial())
        ...
        for d in range(7):
            with st.expander(f"{weekdays[d]}"):
                ...
                s1 = c1.text_input(f"Início faixa 1 (HH:MM) dia {d}", value=st.session_state.get(s1_key, rules[d][0][0] or ""), key=s1_key)
                e1 = c2.text_input(f"Fim faixa 1 (HH:MM) dia {d}", value=st.session_state.get(e1_key, rules[d][0][1] or ""), key=e1_key)
                s2 = c3.text_input(f"Início faixa 2 (HH:MM) dia {d}", value=st.session_state.get(s2_key, (rules[d][1][0] or "")), key=s2_key)
                e2 = c4.text_input(f"Fim faixa 2 (HH:MM) dia {d}", value=st.session_state.get(e2_key, (rules[d][1][1] or "")), key=e2_key)
                st.session_state["rules"][scope_name][selected][d] = [(s1 or None, e1 or None), (s2 or None, e2 or None)]
        st.button("Resetar este item para preset Comercial", on_click=lambda: st.session_state["rules"][scope_name].update({selected: default_rules_comercial()}), key=f"reset_{scope_name}")
        return selected
    ...
    with tabs_rules[3]:
        ...
        if plates_all_for_rules:
            ...
            plate_rules = st.session_state["rules"]["PLATE"].get(prev_plate, default_rules_comercial())
    ...
    with tabs[0]:
        ...
        if segments is None or segments.empty:
            ...
            # Distância em km como número para garantir ordenação correta
            df_disp["Distância (off) (km)"] = df_disp["off_distance_km"].astype(float)
            ...
            })[["UO","Grupo","Motorista","Placa","Início (off)","Fim (off)","Duração (off)","Distância (off) (km)","Velocidade máx. (km/h)","Tipo / Motivo","Severidade"]]
            ...
            st.dataframe(
                ...
                column_config={
                    "Distância (off) (km)": st.column_config.NumberColumn(
                        "Distância (off) (km)",
                        format="%.2f km",
                        help="Distância total fora da janela. Mantida como número (km) para permitir ordenação correta."
                    )
                }
            )
...
"""
    ...
    with tabs[3]:
        ...
        if segments is None or segments.empty:
            ...
            # dias com situações geradas (por placa)
            ...
            if not isinstance(rec["Último evento"].iloc[0], (int,float)):
                rec["Último evento"] = pd.to_datetime(rec["Último evento"], errors="coerce")
            rec["Último evento"] = rec["Último evento"].dt.strftime("%d/%m/%Y %H:%M").fillna("-")
